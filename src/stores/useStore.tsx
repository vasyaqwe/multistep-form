...
export const useStore = create<Store>()((set, get) => ({
    ...
    onNext: () => {
        ...
        const emailInvalidFormat = (field: PersonalInfoField) =>
            field.name === "email" && !emailRegex.test(field.value)
        const phoneInvalidFormat = (field: PersonalInfoField) =>
            field.name === "phone" && !phoneNumberRegex.test(field.value)
        if (
            get().formData.personalInfo.some(
                (field) =>
                    fieldEmpty(field) ||
                    emailInvalidFormat(field) ||
                    phoneInvalidFormat(field)
            )
        ) {
            set((state) => ({
                formData: {
                    ...state.formData,
                    personalInfo: state.formData.personalInfo.map((field) => {
                        if (fieldEmpty(field)) {
                            return {
                                ...field,
                                error: true,
            ...
            if (!get().isLastStep()) {
                set((state) => ({
                    currentStepIdx:
                        state.currentStepIdx >= STEPS_COUNT - 1
                            ? state.currentStepIdx
                            : state.currentStepIdx + 1,
                    animationDirection:
                        state.animationDirection >= STEPS_COUNT - 1
                            ? state.animationDirection
                            : state.animationDirection + 1,
                }))
            } else {
                set(() => ({
                    currentStepIdx: STEPS_COUNT,
                    formSubmitted: true,
                }))
            }
        }
    ...
    onGoTo: (newIdx: number) =>
        set(() => ({ currentStepIdx: newIdx, animationDirection: -1 })),
    ...
    onPlanChange: (e: ChangeEvent<HTMLInputElement>) => {
        const newPlan = get().formData.plans.find(
    ...
    onAddOnChange: (e: ChangeEvent<HTMLInputElement>) => {
        ...
        set((state) => ({
            formData: {
                ...state.formData,
                addOns: state.formData.addOns.map((addOn) =>
