...
export const FinishingUpStep = () => {
    const {
        formData: { billedYearly, plan, addOns },
    } = useStore()

    const pickedAddOns = addOns.filter((addOn) => !addOn.checked)

    const addOnsTotal = pickedAddOns.reduce(
        (acc, curr) => (acc += curr.price),
        0
    )
    const total = billedYearly
        ? (plan.price + addOnsTotal) * 10
        : plan.price + addOnsTotal
        ...
        >
            ...
            <div className="p-4 flex justify-between items-center mt-2">
                <p className="text-neutral-400">
                    Total (per {getPerFullText(billedYearly)})
                </p>
                <small className="text-primary-400 text-lg md:text-xl font-bold">
                    ${total}/{getPerShortText(billedYearly)}
                </small>
            </div>
        </FormWrapper>
    )
