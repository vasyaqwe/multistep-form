...
export const PersonalInfoStep = () => {
    ...
    const inputClassName = (error: boolean) => (
        ...
        ${error ? 'border-secondary-400' : 'focus:border-primary-400'} py-2 px-3 md:py-3`
    )
            ...
            <div className='mt-5'>
                {personalInfo.map((field, idx) => (
                    <div key={field.name} className='mt-5 relative'>
                        {field.error && <span className='text-secondary-400 absolute right-0 font-semibold text-sm md:text-base'>{field.errMsg}</span>}
                        <label htmlFor={field.name} className=' text-primary-900 text-sm md:text-base'>{field.label}</label>
                        <input autoFocus={idx === 0} required className={inputClassName(field.error)} onChange={onChange} value={field.value}
